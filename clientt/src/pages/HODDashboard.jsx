...
//     useEffect(() => {
//         let start = 0;
//         const end = (value,15); // Use the value passed from props
//         const duration = 0.75; // Duration in seconds
...
const Clock = () => {
    ...
    return (
        <div className="text-center mb-6">
            <h2 className="text-2xl font-semibold text-gray-700">Current Time</h2>
            <div className="text-3xl font-bold text-blue-600 mt-2">{formatTime(time)}</div>
        </div>
    );
};
...
const HODDashboard = () => {
    ...
    const events = [
        { color: 'bg-blue-500', title: 'Meet Mr. Vijay and ERP Team to Discuss', date: '09 Nov', time: '12:15 PM - 1:15 PM' },
        { color: 'bg-yellow-500', title: 'Teachers Meeting', date: '07 Nov', time: '12:15 PM - 1:15 PM' },
        { color: 'bg-green-500', title: 'Diwali Holidays', date: 'Oct 28 to Nov 10', time: '' },
    ];
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl flex flex-col md:flex-row justify-between">
                ...
                <div className="flex-1 md:mr-8">
                    ...
                    <h2 className="text-xl font-semibold text-gray-600 mb-4">Upcoming Events/ Holidays</h2>
                    <ul>
                        {events.map((event, index) => (
                            <li key={index} className="flex items-center mb-4">
                                <span className={`w-3 h-3 rounded-full ${event.color} mr-4`}></span>
                                <div>
                                    <h3 className="text-gray-700 font-medium">{event.title}</h3>
                                    <p className="text-sm text-gray-500">{event.date} {event.time && `, ${event.time}`}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
                ...
                {/* Real Calendar Section with Clock */}
                <div className="flex-1 flex flex-col items-center">
                    <Clock /> {/* Add the clock here */}
                    <Calendar
                        onChange={setDate} // When a date is clicked, the state will update
